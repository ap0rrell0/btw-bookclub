...
const BOOKS_INITIAL = [
  ...
  {id:62, month:"March", year:2021, title:"The Sanatorium", author:"Sarah Pearse", genre:"Thriller", clubStatus:"Read", rating:null, notes:null, myStatus:null},
  ...
  {id:65, month:"December", year:2020, title:"Exodus", author:"Kate Stewart", genre:"Romance", clubStatus:"Read", rating:null, notes:null, myStatus:null},
  {id:66, month:"November", year:2020, title:"Flock", author:"Kate Stewart", genre:"Romance", clubStatus:"Read", rating:null, notes:null, myStatus:null},
...
export default function App() {
  ...
  const filtered = books.filter(b => {
    ...
    const matchStatus = filterStatus === "All" || b.myStatus === filterStatus || (!b.myStatus && filterStatus === "No Status") || (filterStatus === "♥ Favourites" && b.favorite);
  ...
  const favouriteCount = books.filter(b => b.favorite).length;
  const favouriteBooks = books.filter(b => b.favorite);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.warmCream, fontFamily: "Georgia,serif", maxWidth: 600, margin: "0 auto", paddingBottom: 80 }}>
      ...
      <div style={{ display: "flex", background: C.wineRed, position: "sticky", top: 0, zIndex: 10 }}>
        {[["list", "Books"], ["add", "Add Book"], ["stats", "Stats"]].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)} style={{ flex: 1, padding: "12px 0", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: tab === key ? "bold" : "normal", color: tab === key ? "#fff" : "rgba(255,255,255,0.5)", borderBottom: tab === key ? "3px solid #fff" : "3px solid transparent", fontFamily: "Georgia,serif" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === "list" && (
        <div style={{ padding: "14px 12px 0" }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
            {["All", ...STATUS_OPTIONS, "No Status", "♥ Favourites"].map(s => {
              const active = filterStatus === s;
              const sm = STATUS_META[s];
              const isFav = s === "♥ Favourites";
              return (
                <button key={s} onClick={() => setFilterStatus(s)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12, cursor: "pointer", fontFamily: "Georgia,serif", fontWeight: active ? "bold" : "normal", border: "1.5px solid " + (isFav && active ? "#C9933A" : active && sm ? sm.border : active ? C.wineRed : "#D4A8B8"), background: isFav && active ? "#FDF3E3" : active && sm ? sm.bg : active ? C.wineRed : "#fff", color: isFav && active ? "#C9933A" : active && sm ? sm.text : active ? "#fff" : C.mutedText }}>
                  {s}
                </button>
              );
            })}
              ...
              <div key={book.id} onClick={() => setExpandedId(isExpanded ? null : book.id)} style={{ background: idx % 2 === 1 ? C.blushRow : "#fff", borderRadius: 10, marginBottom: 8, border: "1px solid #E8C8D0", overflow: "hidden", cursor: "pointer", boxShadow: "0 1px 4px rgba(78,15,36,0.06)" }}>
                <div style={{ display: "flex", alignItems: "center", padding: "11px 14px", gap: 10 }}>
                  ...
                  <span
                    ...
                    style={{ fontSize: 20, flexShrink: 0, cursor: "pointer", color: book.favorite ? "#C9933A" : "#D4A8B8", transition: "transform 0.15s, color 0.15s", display: "inline-block" }}
                  ...
                  <div style={{ borderTop: "1px solid #E8C8D0", padding: "12px 14px", background: C.warmCream }} onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <button
                        ...
                        style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 18px", borderRadius: 20, border: "1.5px solid " + (book.favorite ? "#C9933A" : "#D4A8B8"), background: book.favorite ? "#FDF3E3" : "#fff", color: book.favorite ? "#C9933A" : C.mutedText, cursor: "pointer", fontFamily: "Georgia,serif", fontSize: 14, fontWeight: book.favorite ? "bold" : "normal" }}
                      >
                        <span style={{ fontSize: 20 }}>{book.favorite ? "♥" : "♡"}</span>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {STATUS_OPTIONS.map(s => {
                        ...
                        return (
                          <button key={s} onClick={() => updateMyStatus(book.id, s)} style={{ padding: "7px 14px", borderRadius: 20, fontSize: 12, cursor: "pointer", fontFamily: "Georgia,serif", fontWeight: active ? "bold" : "normal", border: "1.5px solid " + (active ? m.border : "#D4A8B8"), background: active ? m.bg : "#fff", color: active ? m.text : C.mutedText }}>
                            {s}
                          </button>
                        );
                      })}
          ...
          {filtered.length === 0 && <div style={{ textAlign: "center", color: C.mutedText, padding: "40px 0" }}>No books found</div>}
        </div>
      )}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {STATUS_OPTIONS.map(s => {
                  ...
                  return (
                    <button key={s} onClick={() => setAddForm(f => ({ ...f, myStatus: s }))} style={{ padding: "8px 14px", borderRadius: 20, fontSize: 13, cursor: "pointer", fontFamily: "Georgia,serif", fontWeight: active ? "bold" : "normal", border: "1.5px solid " + (active ? m.border : "#D4A8B8"), background: active ? m.bg : "#fff", color: active ? m.text : C.mutedText }}>
                      {s}
                    </button>
                  );
                })}
        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: 20, marginBottom: 14, border: "1px solid #E8C8D0" }}>
            ...
            {favouriteBooks.length === 0 ? (
              ...
              <div>
                <div style={{ fontSize: 12, color: C.mutedText, marginBottom: 10 }}>{favouriteCount} book{favouriteCount !== 1 ? "s" : ""} loved</div>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 88, left: "50%", transform: "translateX(-50%)", background: toast.type === "error" ? C.dnfText : C.deepBurgundy, color: "#fff", padding: "10px 22px", borderRadius: 24, fontSize: 13, zIndex: 999, whiteSpace: "nowrap" }}>
          {toast.msg}
        </div>
      )}
